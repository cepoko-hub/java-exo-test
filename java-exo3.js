...
function script11() {
    let chaine = prompt("Entrez une chaîne de caractères pour vérifier si c'est un palindrome :");

    function palindrome(str) {

        let chaineNettoyee = str.toLowerCase().replace(/[^a-z0-9]/g, "");

        let chaineInversee = chaineNettoyee.split("").reverse().join("");
        return chaineNettoyee === chaineInversee;
    }

    let estPalindrome = palindrome(chaine);
